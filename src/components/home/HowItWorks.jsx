      ...
      <div>
        ...
        <div className="flex items-center justify-evenly">
          {GUIDE.map((item, index) => (
            <article
              key={index}
              className={`flex flex-col rounded-lg px-4 py-8 border-2 w-1/6 gap-2 text-center hover:scale-105 hover:-translate-y-2 hover:shadow-2xltransition-transform ${borderColors[index]} ${bgColors[index]}`}>
              <p>{item.step}</p>
              <div>{item.image}</div>
